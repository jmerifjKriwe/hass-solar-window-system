"""
PoC snapshot test that serialises a real Home Assistant State object.

This demonstrates using the real ``hass.states.get`` State object with
``serialize_entity`` and the snapshot helper.
"""
...
@pytest.mark.asyncio
async def test_hass_state_serialization(hass: HomeAssistant) -> None:
