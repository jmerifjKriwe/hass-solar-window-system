"""Test the switch platform setup for Solar Window System (migrated)."""
...
@pytest.mark.asyncio
async def test_switch_platform_setup(hass) -> None:
    entry = MockConfigEntry(
        domain=DOMAIN,
        title="Solar Window System",
        data={"entry_type": "global_config"},
        entry_id="test_switch_setup",
    )
    ...
    if len(added_entities) == 0:
        raise AssertionError("No switch entities registered")
...
@pytest.mark.asyncio
async def test_switch_platform_setup(hass: HomeAssistant) -> None:
    """Ensure switch entities are registered."""
    entry = MockConfigEntry(
        domain=DOMAIN,
        title="Solar Window System",
        data={"entry_type": "global_config"},
